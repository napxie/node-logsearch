/*! 基于HTML5表单验证jQuery插件,支持标准的Html5验证方式
 ...
 * 自定义提标信息 data-message
 * 调用方式
 * $(form).html5Validate(callback, options);
 * 在options里定义自定义验证及提示
 * options = {validate:function() {
 *     if (condition) {
 *         $(selector).customTips("此项自定义验证未通过");
 *         return false;
 }
 return true;
 }
...
(function ($, undefined) {
    ...
    // 自定义提示隐藏、绑定事件以及解绑事件
    $.validateTips = (function () {
        var winWidth = $(window).width();
        var fnMouseDown = function (e) {
            if (!e || !e.target) return;
            if (e.target.id !== $.validateTips.id && $(e.target).parents("#" + $.validateTips.id).length === 0) {
                $.validateTips.hide();
            }
            ...
            if (!$.validateTips.display) return;
    ...
    $.html5Validate = (function () {
        ...
        return {
            ...
            isOverflow:function (ele) {
                ...
                if (!attrMin && !attrMax) {
                    ...
                    if (attrMin && value < attrMin) {
                        $(ele).tips("值必须大于或等于" + attrMin);
                    } else if (attrMax && value > attrMax) {
                        $(ele).tips("值必须小于或等于" + attrMax);
            ...
            isAllpass:function (elements, options) {
                ...
                var self = this, allpass = true;
                var remind = function (control, type, tag) {
                    var key = $(control).attr("data-message"), text = '', placeholder;

                    // 如果元素完全显示
                    if ($(control).isVisible()) {
                        if (type == "radio" || type == "checkbox") {
                            $(control).tips(OBJREG.prompt[type], {
                                align:"left"
                            });
                            control.focus();
                        } else if (tag == "select" || tag == "empty") {
                            // 下拉值为空或文本框文本域等为空
                            $(control).tips((tag == "empty" && text) ? "您尚未输入" + text : OBJREG.prompt[tag]);
                            control.focus();
    ...
    $.fn.extend({
        ...
        tips:function (content, options) {
            var ele = $(this);
            var message = $.html5Attr($(this), 'data-message');
            var placeholder = $.html5Attr($(this), "placeholder");
            var value = $(this).val();
            var trimValue = value;
            if (ele.type !== "password") {
                trimValue = $.trim(trimValue);
            ...
            var empty = false;
            if (trimValue === "" || trimValue === placeholder) empty = true;
            //有自定义提示且值不为空时，提示自定义信息
            if (message) {
            ...
            //值为空且设置了placeholder时，提示信息为plackholder
            ...
            options = options || {};
            options.css = $.extend({}, defaults.css, options.css || $.validateTips.css);
            ...
            if ($("#" + $.validateTips.id).size()) $.validateTips.hide();

            //如果有自定义的data-message,那么提示只显示自定义信息

            this.remind = $('<div id="' + $.validateTips.id + '">' + content + '</div>').css(params.css);
        ...
        customTips:function (content) {
            this.tips(content, {custom:true});
        },
